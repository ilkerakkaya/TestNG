...
public abstract class TestBase {
    ...
    public void setupMethod() {
        extentTest=extentReports.createTest("Koala Application Reports");
    ...
    public void setUpTest() {//This is how to set up Extent report. We will create and use this one in out test classes
        ...
        extentHtmlReporter.config().setReportName("Koala Automation Test Reports");
    ...
    public void tearDownMethod(ITestResult result) throws Exception {
        if (result.getStatus() == ITestResult.FAILURE) {//When test case fails, then take the screenshot and attached the report
            ...
            extentTest.fail(result.getName()).addScreenCaptureFromPath(screenshotLocation);
            extentTest.fail(result.getThrowable());
        } else if (result.getStatus() == ITestResult.SKIP) {
            String screenshotLocation = ReusableMethods.getScreenshot(result.getName());
            extentTest.skip(result.getName()).addScreenCaptureFromPath(screenshotLocation);
